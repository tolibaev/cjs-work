...
export function applyStyles(rootElement, options) {
  ...
  for (let element of elements) {
    ...
    for (className of classes) {
      ...
      callBuilderFunctionsWithoutNumbresValue(arrayWithCssStylesWithoutNumbersValue, builderPropertyWithoutNumbresValues);
    }
    for (let key in dataAttributes) {
      const value = dataAttributes[key];
      const valuesArray = value.split(' ');
      valuesArray.forEach(property => {
        const objectWithPropsAndAbbr = [...arrayWidthDisplayProprties, ...valuesFromDataAttributes].find(valueFromDataAttribute => {
        ...
        function setPseudoClasses(key, storageVariable) {
          ...
          if (objectWithPropsAndAbbr.abbreviation === 'translateX' || objectWithPropsAndAbbr.abbreviation === 'translateY') {
          ...
          if (objectWithPropsAndAbbr.abbreviation === 'scaleX' || objectWithPropsAndAbbr.abbreviation === 'scaleY' || objectWithPropsAndAbbr.abbreviation === 'scaleZ') {
          ...
          if (objectWithPropsAndAbbr.abbreviation === 'rotateX' || objectWithPropsAndAbbr.abbreviation === 'rotateY' || objectWithPropsAndAbbr.abbreviation === 'rotateZ') {
            const value = removeLetters(propertyValue.split('-'));
            style = `${value}deg`;
            storageVariable += `${objectWithPropsAndAbbr.property}: ${objectWithPropsAndAbbr.abbreviation}(${style});`;
          }
          ...
          if (objectWithPropsAndAbbr.abbreviation === 'skewX' || objectWithPropsAndAbbr.abbreviation === 'skewY' || objectWithPropsAndAbbr.abbreviation === 'skewZ') {
            const values = removeLetters(propertyValue.split('-'));
            style = `${values}deg`;
            storageVariable += `${objectWithPropsAndAbbr.property}: ${objectWithPropsAndAbbr.abbreviation}(${style});`;
          }

          if (objectWithPropsAndAbbr.abbreviation === 'hue-rot' || objectWithPropsAndAbbr.abbreviation === 'bd-hue-rot') {
            const values = removeLetters(propertyValue.split('-'));
            style = `${values}deg`;
          ...
          if (objectWithPropsAndAbbr.abbreviation === 't-shadow' || objectWithPropsAndAbbr.abbreviation === 'd-shadow') {
            const values = removeLetters(propertyValue.split('-'));
            ...
            if (objectWithPropsAndAbbr.abbreviation === 'd-shadow') storageVariable += `${objectWithPropsAndAbbr.property}: drop-shadow(${style});`;
          }
          ...
          if (objectWithPropsAndAbbr.abbreviation === 'm-' || objectWithPropsAndAbbr.abbreviation === 'p-' || objectWithPropsAndAbbr.abbreviation === 'w-' || objectWithPropsAndAbbr.abbreviation === 'h-') {
            propertyValue = property.replace(`${objectWithPropsAndAbbr.abbreviation}`, '');
            const values = removeLetters(propertyValue.split('-'));
          ...
          if (objectWithPropsAndAbbr.abbreviation === 'g-cols' || objectWithPropsAndAbbr.abbreviation === 'g-rows') storageVariable += `${objectWithPropsAndAbbr.property}: repeat(${propertyValue}, minmax(0, 1fr))`;

          if (objectWithPropsAndAbbr.property.includes('display')) storageVariable += `${objectWithPropsAndAbbr.property};`;
  ...
  function builderPropertyWithoutNumbresValues(property, abbreviationForProperty) {
    ...
    if (className.startsWith(`${abbr}`)) {
      ...
      if (abbr === 'mx') {
        ...
        if (options.unit === 'rem') setCssStyleInLocalStorage(`${property}-left:${propertyValue / htmlFontSize}rem;${property}-right:${propertyValue / htmlFontSize}rem;`);
      }

      if (abbr === 'my') {
        setCssStyleInLocalStorage(`${property}-top:${propertyValue}${options.unit};${property}-bottom:${propertyValue}${options.unit};`);
        if (options.unit === 'rem') setCssStyleInLocalStorage(`${property}-top:${propertyValue / htmlFontSize}rem;${property}-bottom:${propertyValue / htmlFontSize}rem;`);
      }
      ...
      if (abbr === 'color' || abbr === 'bg-color' || abbr === 'bor-color' || abbr === 'out-color' || abbr === 'accent' || abbr === 'caret-color') setCssStyleInLocalStorage(`${property}: #${propertyValue}`);

      if (abbr === 'g-flow' || abbr === 'flex-dir' || abbr === 'obj-pos') setCssStyleInLocalStorage(`${property}: ${propertyValue.split('-').join(' ')}`);
      if (abbr === 'bg-pos') {
        ...
        if (propertyValue.length === 1) {
          ...
          if (options.unit === 'rem') setCssStyleInLocalStorage(`${property}: ${value / htmlFontSize}${options.unit}`);
        }
        if (propertyValue.length === 2) {
          const [valueX, valueY] = propertyValue;
          setCssStyleInLocalStorage(`${property}: ${valueX}${options.unit} ${valueY}${options.unit}`);
          if (options.unit === 'rem') setCssStyleInLocalStorage(`${property}: ${valueX / htmlFontSize}${options.unit} ${valueY / htmlFontSize}${options.unit}`);
        }
      }
      if (abbr === 'bor' || abbr === 'bor-top' || abbr === 'bor-right' || abbr === 'bor-left' || abbr === 'bor-bottom') {
        ...
        if (options.unit === 'rem') setCssStyleInLocalStorage(`${property}: ${borderWidth / htmlFontSize}rem ${borderStyle} #${borderColor}`);
      }
      // transition
      ...
      if (abbr === 'trans-fun') {
        if (className.includes('cub')) {
          ...
          localStorage.setItem(`${className}`, `.${className.replace(/\(/g, '\\(').replace(/\)/g, '\\)')}{${property}: cubic-bezier(${innerValues.join(', ')})}`);
        }
      }

      if (abbr === 'jus-con' || abbr === 'align-con' || abbr === 'al-items' || abbr === 'al-self' || abbr === 'place-con' || abbr === 'aspect') {
        ...
        if (propertyValue.match(/^\d+.\d+$/)) setCssStyleInLocalStorage(`${property}: ${propertyValue.split('-').join('/')}`);
      }

      if (abbr === 'container') {
        localStorage.setItem(className, `.${className}{${property}: ${options.containerSize}${options.unit};margin: 0 auto;padding: 0 ${options.containerPadding}${options.unit}}`);
        if (options.unit === 'rem') {
          localStorage.setItem(className, `.${className}{${property}: ${options.containerSize / htmlFontSize}rem;margin: 0 auto;padding: 0 ${options.containerPadding / htmlFontSize}rem}`);
        }
      }

      if (abbr === 'auto-cols' || abbr === 'auto-rows') {
        setCssStyleInLocalStorage(`${property}: ${propertyValue === 'fr' ? (propertyValue = 'minmax(0, 1fr)') : propertyValue}`);
      }

      if (abbr === 'line-clamp') {
        setCssStyleInLocalStorage(`	overflow: hidden;display: -webkit-box;-webkit-box-orient: vertical;${property}: ${propertyValue}`);
      }
  ...
  function builderTransformProperties(property, abbreviationForProperty, unit) {
    if (className.startsWith(`${abbreviationForProperty}-`)) {
      ...
      if (!valueName.includes('rotate') && !valueName.includes('scale') && !valueName.includes('t-origin') && !valueName.includes('hue-rot')) {
        ...
        if (abbr === 'translate') {
          const values = removePropertyName(className, abbr).split('-');
          ...
          if (filteredValues.length === 2) {
            ...
            if (String(firstValue).match(/(vw|vh|dvw|dvh|%)$/) && String(secondValue).match(/(vw|vh|dvw|dvh|%)$/)) {
              propertyValue = `${firstValue} ${secondValue}`;
              localStorage.setItem(className, `.${className}{${valueName}: ${propertyValue}}`);
            } else {
              propertyValue = `${firstValue}${options.unit} ${secondValue}${options.unit}`;

              if (unit === 'rem') propertyValue = `${firstValue / htmlFontSize}rem ${secondValue / htmlFontSize}rem`;
        ...
        if (abbr === 'translateX' || abbr === 'translateY') {
          const values = removePropertyName(className, abbr).split('-');
          ...
          if (filteredValues.length === 1) {
            ...
            if (String(firstValue).match(/(vw|vh|dvw|dvh|%)$/)) {
              propertyValue = `${firstValue}`;
              localStorage.setItem(className, `.${className}{${property}:${valueName}(${propertyValue})}`);
            } else {
              propertyValue = `${firstValue}${options.unit}`;
      ...
      if (valueName.includes('skew')) localStorage.setItem(className, `.${className}{${property}: ${valueName}(${secendValue}deg)}`);

      if (valueName.includes('rotate')) localStorage.setItem(className, `.${className}{${valueName}:${secendValue}deg}`);

      if (abbr.includes('hue-rot') || abbr.includes('bd-hue-rot')) localStorage.setItem(className, `.${className}{${property}: hue-rotate(${secendValue}deg)}`);

      if (valueName === 'scale') localStorage.setItem(className, `.${className}{${valueName}: ${secendValue / 10}}`);
      if (valueName === 'scaleX' || valueName === 'scaleY' || valueName === 'scaleZ') localStorage.setItem(className, `.${className}{${property}: ${abbr}(${secendValue / 10})}`);
      if (valueName.includes('t-origin')) {
        const [secendValue] = removePropertyName(values);
        localStorage.setItem(className, `.${className}{${property}: ${valueName}(${secendValue / 10})}`);
      }
    }
  }
  function builderFilterProperties(property, abbreviationForProperty, unit) {
    if (className.startsWith(`${abbreviationForProperty}-`)) {
      ...
      // Only filter
      if (valueName.includes('saturate') || valueName.includes('brightness') || valueName.includes('contrtas') || valueName.includes('opacity')) propertyValue = `${valueName}(${firstValue / 100})`;

      if (valueName === 'grayscale' || valueName === 'invert' || valueName === 'sepia') propertyValue = `${valueName}(${firstValue}%)`;
      ...
      if (valueName === 'bd') {
        ...
        if (className.includes('grayscale') || className.includes('invert') || className.includes('sepia')) elementStyle = `${property}: ${propertyName}(${value}%)`;
      }
  ...
  function conditionsForAddingNewPropertiesWithMoreValue(property, abbreviationForProperty, unit, condition) {
    ...
    if (className.startsWith(`${abbr}-`)) {
      ...
      if (filteredValues.length === 1) {
        ...
        if (abbr === 'font-w' || abbr === 'grow' || abbr === 'shrink' || abbr === 'order' || abbr === 'row-start' || abbr === 'row-end' || abbr === 'col-start' || abbr === 'col-end') propertyValue = `${firstValue}`;
      ...
      if (filteredValues.length === 2) {
        ...
        if (firstValue > secondValue) {
          ...
          if (unit === 'rem') {
            propertyValue = `clamp(${secondValue / htmlFontSize}rem, calc(${secondValue / htmlFontSize}rem + (${firstValue} - ${secondValue}) * ((100vw - ${options.finalWidth / htmlFontSize}rem) / (${rangeWidth}))), ${firstValue / htmlFontSize}rem)`;
          }
        }
        ...
        if (firstValue < secondValue) {
          ...
          if (unit === 'rem') {
            propertyValue = `clamp(${firstValue / htmlFontSize}rem, calc(${secondValue / htmlFontSize}rem - (${secondValue} - ${firstValue}) * ((100vw - ${options.finalWidth / htmlFontSize}rem) / (${rangeWidth}))), ${secondValue / htmlFontSize}rem)`;
          }
        }
        conditionDirection();
      }
      ...
      if (filteredValues.length === 4) {
        ...
        if (firstValue > secondValue) {
          ...
          if (unit === 'rem') {
            propertyValue = `clamp(${secondValue / htmlFontSize}rem, calc(${secondValue / htmlFontSize}rem + (${firstValue} - ${secondValue}) * ((100vw - ${endWidth / htmlFontSize}rem) / (${rangeWidth}))), ${firstValue / htmlFontSize}rem)`;
          }
        }
        ...
        if (firstValue < secondValue) {
          ...
          if (unit === 'rem') {
            propertyValue = `clamp(${firstValue / htmlFontSize}rem, calc(${secondValue / htmlFontSize}rem - (${secondValue} - ${firstValue}) * ((100vw - ${endWidth / htmlFontSize}rem) / (${rangeWidth}))), ${secondValue / htmlFontSize}rem)`;
          }
        }
      ...
      if (abbr === 'radius') {
        ...
        if (filteredValues.length === 1) {
          let [all] = filteredValues;

          propertyValue = `${all}${options.unit}`;
          if (options.unit === 'rem') {
  ...
  function removeLetters(values) {
    ...
    const filteredValues = values.filter(value => {
      ...
      if (negativeValues.test(value) || positiveValues.test(value) || positiveRelativeValues.test(value) || negativeRelativeValues.test(value)) {
        return true;
      }
    });
